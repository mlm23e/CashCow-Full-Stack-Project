...
from app.models import User, UserRole, Branch, ServiceCall, ServiceStatus
from app.schemas.user import UserRead, UserUpdate, UserCreate, TechnicianActiveCallsRead
...
@router.get("/active-calls/{supervisor_id}", response_model=list[TechnicianActiveCallsRead])
async def technicians_with_active_calls(
    supervisor_id: int,
    db: AsyncSession = Depends(get_db),
    _: User = Depends(get_current_user),
) -> list[TechnicianActiveCallsRead]:
    active_call_count = func.count(ServiceCall.id)
    statement = (
        select(
        ...
        .join(ServiceCall, ServiceCall.technician_id == User.id)
        .where(
